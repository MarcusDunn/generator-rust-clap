//! Generation entry point. Phase 1: tag-grouped subcommand emission
//! atop the iteration-2 reqwest client. Specs with no tags collapse to
//! a flat enum (same as before); specs with tags emit nested clap
//! Args/Cmd type pairs.
...
/// Emit the root `enum Cmd { ... }`. Roots that are the `_misc`
/// catch-all collapse: their direct ops become variants of `Cmd`
/// directly. Other roots become `<RootPascal>(<RootPascal>Args)`
/// variants.
fn emit_root_enum(out: &mut String, tree: &TagTree) {
    out.push_str("#[derive(Subcommand)]\nenum Cmd {\n");
    for root in &tree.roots {
        if root.is_misc() {
...
/// Recursively emit `<Q>Args` struct + `<Q>Cmd` enum for `group` and
/// every descendant. `prefix` is the qualified PascalCase prefix from
/// ancestors (empty for roots).
fn emit_group_types(out: &mut String, group: &TagGroup, prefix: &str) {
    if group.is_misc() {
        return; // _misc is inlined at the root; no nested types.
    }
...
/// Emit match arms for the root `Cmd` enum. `_misc` direct ops appear
/// as direct `Cmd::<OpVariant>` arms; non-misc roots emit a single
/// `Cmd::<Variant>(g) => { ... }` arm whose body recurses through the
/// nested enums.
